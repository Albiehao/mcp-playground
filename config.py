...
default_mcp_prompts = {
    ...
    [get_text('amap', '高德地图')]: ['北京今天天气怎么样', '基于今天的天气，帮我规划一条从北京到杭州的路线'],
    '可视化图表': ['生成一个柱状图模拟出不同城市的销售数据', '生成一个折线图模拟出过去一年每个月的气温变化趋势'],
...
bot_avatars = {
    "Qwen": os.path.join(os.path.dirname(__file__), "./assets/qwen.png"),
    "QwQ": os.path.join(os.path.dirname(__file__), "./assets/qwen.png"),
    "LLM-Research": os.path.join(os.path.dirname(__file__),
                                 "./assets/meta.webp"),
    "deepseek-ai": os.path.join(os.path.dirname(__file__),
                                "./assets/deepseek.png"),
    "Menlo": os.path.join(os.path.dirname(__file__), "./assets/menlo.png"),
}
...
def user_config(disabled_actions=None):
    return ChatbotUserConfig(actions=[
        ...
        ChatbotActionConfig(action='delete',
                            popconfirm=dict(
                                title=get_text('Delete Message', '删除消息'),
